...
export default function EditFuelScreen() {
  ...
  const { fuelEntries, updateFuelEntry } = useFuel();
  ...
  useEffect(() => {
    if (entry) {
      setTruckId(entry.truckId || '');
      setDate(entry.date);
      setGallons(entry.gallons.toString());
      setPricePerGallon(entry.pricePerGallon.toString());
      setOdometer(entry.odometer.toString());
      setLocation(entry.location || '');
      setIsFillUp(entry.isFillUp);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to update fuel entry. Please try again.');
    } finally {
